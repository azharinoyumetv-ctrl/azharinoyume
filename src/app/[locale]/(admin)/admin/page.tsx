...
async function getDashboardStats() {
  ...
  const [
    totalOrders, activeOrders, pendingWise, todayRevenue, monthRevenue,
    recentOrders, costWarnings,
  ] = await Promise.all([
    prisma.order.count(),
    prisma.order.count({ where: { status: { notIn: ["delivered", "failed", "draft", "awaiting_payment"] } } }),
    prisma.order.count({ where: { manualReviewRequired: true, adminApproved: false } }),
    prisma.ledgerEntry.aggregate({ where: { entryType: "revenue", createdAt: { gte: startOfDay } }, _sum: { amount: true } }),
  ...
  return { totalOrders, activeOrders, pendingWise, todayRevenue: Number(todayRevenue._sum.amount || 0), monthRevenue: Number(monthRevenue._sum.amount || 0), recentOrders, costWarnings };
}
    ...
    <div>
      ...
      {stats.pendingWise > 0 && (
        <div className="flex items-center gap-3 p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 mb-8">
          ...
          <div>
            <div className="font-semibold text-amber-400">{stats.pendingWise} Wise payment{stats.pendingWise > 1 ? "s" : ""} awaiting approval</div>
            <div className="text-sm text-amber-400/70">Manual review required before processing can start.</div>
          </div>
          <Link href="/admin/payments?filter=wise_pending" className="ml-auto px-4 py-2 bg-amber-500 text-black rounded-lg text-sm font-bold">
            Review Now
          </Link>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { label: "Today's Revenue", value: formatCurrency(stats.todayRevenue), icon: TrendingUp, color: "text-green-400" },
          { label: "Month Revenue", value: formatCurrency(stats.monthRevenue), icon: TrendingUp, color: "text-gold-400" },
              ...
              <Link key={order.id} href={`/admin/orders/${order.id}`} className="flex items-center justify-between px-6 py-4 hover:bg-white/2 transition-colors">
                ...
                <div className="text-right">
                  ...
                  <div className={`text-xs mt-0.5 ${
                    ...
                    order.status === "draft_ready" ? "text-gold-400" :
